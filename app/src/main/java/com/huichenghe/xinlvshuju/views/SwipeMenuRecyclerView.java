 ...
 */
package com.huichenghe.xinlvshuju.views;
...
import com.huichenghe.xinlvshuju.views.touch.OnItemMoveListener;
import com.huichenghe.xinlvshuju.views.touch.OnItemMovementListener;
import com.huichenghe.xinlvshuju.views.touch.OnItemStateChangedListener;
...
{
    public final String TAG = "SwipeMenuRecyclerView";
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        Log.i(TAG, "滑动的距离onInterceptTouchEvent:" + isIntercepted);
        if (!isInterceptTouchEvent) {
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    Log.i(TAG, "滑动的距离isIntercepted:前" + isIntercepted);
                    isIntercepted = handleUnDown(x, y, isIntercepted);
                    Log.i(TAG, "滑动的距离isIntercepted:后" + isIntercepted);
    ...
    {
        // true 列表滑动，false item滑动
        int disX = mDownX - x;
        int disY = mDownY - y;
//        Log.i(TAG, "滑动的距离00x:" + disX + "滑动的距离y:" + disY);
        ...
        // swipe
        if (Math.abs(disX) > mViewConfig.getScaledTouchSlop() && Math.abs(disY) < mViewConfig.getScaledTouchSlop() * 7)
        {
//            Log.i(TAG, "滑动的距离01:" + disX + "滑动的距离ScaledTouchSlop:" + mViewConfig.getScaledTouchSlop());
            defaultValue = false;
        }
        // click
        if (Math.abs(disY) < mViewConfig.getScaledTouchSlop() && Math.abs(disX) < mViewConfig.getScaledTouchSlop())
        {
//            Log.i(TAG, "滑动的距离02:" + Math.abs(disY) + "滑动的距离Math.abs(disX):" + Math.abs(disX));
            defaultValue = false;
        }
        return defaultValue;
    }
